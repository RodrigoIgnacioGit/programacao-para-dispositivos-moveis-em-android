...
export default function TabTwoScreen2() {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      const storedProducts = await AsyncStorage.getItem('tabTwo2_product_quantity_${id}');
      if (storedProducts) {
        setProducts(JSON.parse(storedProducts));
      } else {
        setProducts(initialProducts);
        await AsyncStorage.setItem('tabTwo2_product_quantity_${id}', JSON.stringify(initialProducts));
      }
    };
    loadProducts();
  }, []);
  ...
  const atualizarQuantidade = async (id, newQuantity) => {
    ...
    await AsyncStorage.setItem('tabTwo2_product_quantity_${id}', JSON.stringify(updatedProducts));
  };
